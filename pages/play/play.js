...
Page({
  ...
  onLoad: function (options) {
    ...
    let song_info = wx.getStorageSync('audioList')
    let i = 0
    ...
    audioManger.title = "test" //song_info[curr].title
  ...
  formersong: function () {
    ...
    let song_info = wx.getStorageSync("audioList")[pos]
    // audioManger.src =
    // audioManger.title = song_info.title
    audioManger.title = "test"
  ...
  toMyCollect: function () {
    ...
    if (!isClollect) {
      //没被收藏
      let song_info = wx.getStorageSync('audioList')[curr]
      cl.push(song_info)
    } else {
      // cl.slice(index_item)
